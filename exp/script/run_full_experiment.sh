...
do
    ...
    do
        ...
        do
            for n_total_round in "${n_total_round_list[@]}"
            do
                python script/utility_experiment.py --dataset_name=$dataset_name --agg_strategy=$method --n_users=$n_users --n_total_round=$n_total_round --times=$TIMES --version=$VERSION
            done
        done
    done
done
...
do
    ...
    do
        ...
        do
            for n_total_round in "${n_total_round_list[@]}"
            do
                python script/utility_experiment.py --dataset_name=$dataset_name --agg_strategy=ULDP-GROUP --n_users=$n_users --group_k=$group_k --n_total_round=$n_total_round --times=$TIMES --version=$VERSION
            done
        done
    done
done
