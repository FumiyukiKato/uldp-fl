...
RUN_SIMULATION_PATH="$SCRIPT_PATH/../../../src/run_uldp_vs_puldp.py"
...
for ((j=0; j<$n_users_length; j++)); do
    for ((i=0; i<$dist_length; i++)); do
        ...
        if ((counter >= RESTART)); then
            python $RUN_SIMULATION_PATH --dataset_name=heart_disease --verbose=1 --q_step_size=0.8 --n_users=$n_users --sigma=1.0 --global_learning_rate=10.0 --n_total_round=50 --local_learning_rate=0.001 --local_epochs=50 --user_dist=$user_dist --user_alpha=0.5 --silo_dist=$silo_dist --silo_alpha=2.0 --times=$TIMES --version=$VERSION $DRY_RUN
        fi
        ((counter+=1))
    done
done
